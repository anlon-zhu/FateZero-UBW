...
IMAGE_EXTENSION = (".jpg", ".jpeg", ".png", ".ppm",
                   ".bmp", ".pgm", ".tif", ".tiff", ".webp", ".JPEG")
...
def pad(image: Image.Image, top=0, right=0, bottom=0, left=0,
        color=(255, 255, 255)) -> Image.Image:
    new_image = Image.new(
        image.mode,
        (image.width + right + left, image.height + top + bottom),
        color)
...
def make_grid(
        ...
        rows=None, cols=None) -> Image.Image:
...
def save_images_as_folder(
    ...
    for index, image in enumerate(images):
        ...
        if len(np.array(init_image).shape) == 3:
            cv2.imwrite(
                os.path.join(save_path, f"{index:05d}.png"),
                np.array(init_image)[:, :, :: -1])
        else:
            cv2.imwrite(
                os.path.join(save_path, f"{index:05d}.png"),
                np.array(init_image))
...
def log_train_samples(
    ...
    train_samples = [
        make_grid(
            images,
            cols=int(
                np.ceil(np.sqrt(len(train_samples)))))
        for images in zip(*train_samples)]
...
def save_gif_mp4_folder_type(images, save_path, save_gif=True):
    ...
    if isinstance(images[0], np.ndarray):
        ...
        images = [transforms.ToPILImage()(i.cpu().clone()[0])
                  for i in images]
...
def convert_mp4_to_folder(mp4_path, output_folder):
    ...
    for frame_index in range(frame_count):
        ...
        frame_path = os.path.join(
            output_folder, f"frame_{frame_index:05d}.png")
