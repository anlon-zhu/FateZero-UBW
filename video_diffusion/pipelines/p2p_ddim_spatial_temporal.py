...
class P2pDDIMSpatioTemporalPipeline(
        SpatioTemporalStableDiffusionPipeline):
    def __init__(
            self, vae: AutoencoderKL, text_encoder: CLIPTextModel,
            tokenizer: CLIPTokenizer, unet: UNetPseudo3DConditionModel,
            scheduler:
            Union
    ...
    def check_inputs(
            self, prompt, height, width, callback_steps, strength=None):
    ...
    @torch.no_grad()
    def prepare_latents_ddim_inverted(
            self, image, batch_size, num_images_per_prompt,
            text_embeddings, store_attention=False, prompt=None,
            generator=None, LOW_RESOURCE=True, save_path=None):
        self.prepare_before_train_loop()
        if store_attention:
            attention_util.register_attention_control(
                self, self.store_controller)
        resource_default_value = self.store_controller.LOW_RESOURCE
        # in inversion, no CFG, record all latents attention
        ...
        if isinstance(generator, list):
            init_latents = [self.vae.encode(
                image[i: i + 1]).latent_dist.sample(generator[i]) for i in range(batch_size)]
        ...
        # get latents
        init_latents_bcfhw = rearrange(
            init_latents, "(b f) c h w -> b c f h w", b=batch_size)
        ddim_latents_all_step = self.ddim_clean2noisy_loop(
            init_latents_bcfhw, text_embeddings, self.store_controller)
        if store_attention and (save_path is not None):
            os.makedirs(save_path+'/cross_attention')
            attention_output = attention_util.show_cross_attention(
                self.tokenizer, prompt, self.store_controller, 16,
                ["up", "down"],
                save_path=save_path + '/cross_attention')

            # Detach the controller for safety
            attention_util.register_attention_control(
                self, self.empty_controller)
        self.store_controller.LOW_RESOURCE = resource_default_value

        return ddim_latents_all_step

    @torch.no_grad()
    def ddim_clean2noisy_loop(
            self, latent, text_embeddings,
        ...
        for i in trange(len(self.scheduler.timesteps)):
            t = self.scheduler.timesteps[len(
    ...
    def next_clean2noise_step(
            self, model_output: Union[torch.FloatTensor, np.ndarray],
            timestep: int, sample: Union
        ...
        next_sample_direction = (
            1 - alpha_prod_t_next) ** 0.5 * model_output
    ...
    def p2preplace_edit(self, **kwargs):
        ...
        print(
            f" len_source: {len_source}, len_target: {len_target}, equal_length: {equal_length}")
        edit_controller = attention_util.make_controller(
            self.tokenizer,
            [kwargs['source_prompt'],
             kwargs['prompt']],
            NUM_DDIM_STEPS=kwargs['num_inference_steps'],
            is_replace_controller=kwargs.get(
                'is_replace_controller', True) and equal_length,
    ...
    def __call__(self, **kwargs):
        ...
        if edit_type == 'save':
            ...
            attention_util.register_attention_control(
                self, self.store_controller)
            sdimage_output = self.sd_ddim_pipeline(
                controller=self.store_controller, **kwargs)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(tqdm(timesteps)):
                ...
                noise_pred = self.unet(
                    ...
                    encoder_hidden_states=text_embeddings).sample.to(
                    dtype=latents_dtype)
                ...
                if i == len(timesteps) - 1 or ((i + 1) >
                                               num_warmup_steps and (i + 1) % self.scheduler.order ==
                                               0):
                    progress_bar.update()
    ...
    def print_pipeline(self, logger):
        ...
        expected_modules, optional_parameters = self._get_signature_keys(
            self)
        components_details = {k: getattr(self, k)
                              for k in self.config.keys()
                              if
                              not k.startswith("_") and k
                              not in optional_parameters}
        import json
