...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def step_callback(self, x_t):
        ...
        if self.latent_blend is not None:
            if self.use_inversion_attention:
                step_in_store = len(
                    self.additional_attention_store.latents_store) - self.cur_step
            else:
                step_in_store = self.cur_step

            inverted_latents = self.additional_attention_store.latents_store[step_in_store]
            inverted_latents = inverted_latents.to(
                device=x_t_device, dtype=x_t_dtype)
            ...
            step_in_store_atten_dict = self.additional_attention_store.attention_store_all_step[
                step_in_store]
            if isinstance(step_in_store_atten_dict, str):
                step_in_store_atten_dict = torch.load(
            ...
            for key in blend_dict.keys():
                ...
                for i, attention in enumerate(
                        place_in_unet_cross_atten_list):

                    concate_attention = torch.cat(
                        [attention[None, ...], self.attention_store[key][i][None, ...]], dim=0)
                    blend_dict[key].append(
                        copy.deepcopy(concate_attention))
            x_t = self.latent_blend(x_t=copy.deepcopy(
                torch.cat(
                    [inverted_latents, x_t],
                    dim=0)),
                attention_store=copy.deepcopy(
                blend_dict))
            return x_t[1:, ...]
        else:
            return x_t
    ...
    def replace_self_attention(
            self, attn_base, att_replace, reshaped_mask=None):
        if att_replace.shape[-2] <= 32 ** 2:
            ...
            if reshaped_mask is not None:
                return_attention = reshaped_mask * \
                    att_replace + (1-reshaped_mask)*attn_base
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str):
        ...
        if attn.shape[-2] <= 32 ** 2:
            ...
            if self.use_inversion_attention:
                step_in_store = len(
                    self.additional_attention_store.attention_store_all_step) - self.cur_step - 1
            else:
                step_in_store = self.cur_step
            ...
            if is_cross or (
                    self.num_self_replace[0] <= self.cur_step < self.
                    num_self_replace[1]):
                clip_length = attn.shape[0] // (self.batch_size)
                attn = attn.reshape(
                    self.batch_size, clip_length, *attn.shape[1:])
                ...
                if is_cross:
                    ...
                    # b t h p n = [1, 1, 8, 1024, 77]
                    attn[0:] = attn_repalce_new
                else:
                    ...
                    if self.attention_blend is not None and attn_repalce.shape[-2] <= 32 ** 2:
                        ...
                        w = h
                        mask = self.attention_blend(
                            target_h=h, target_w=w,
                            attention_store=step_in_store_atten_dict,
                            step_in_store=step_in_store)
                        # reshape from ([ 1, 2, 32, 32]) -> [2, 1, 1024, 1]
                        reshaped_mask = rearrange(
                            mask, "d c h w -> c d (h w)")[..., None]
                        ...
                        reshaped_mask = None
                    attn[0:] = self.replace_self_attention(
                        attn_base, attn_repalce, reshaped_mask)

                attn = attn.reshape(
                    self.batch_size * clip_length, *attn.shape[2:])
                # save in format of [temporal, head, resolution, text_embedding]
...
class AttentionReplace(AttentionControlEdit):
    ...
    def replace_cross_attention(self, attn_base, att_replace):
        ...
        if attn_base.dim() == 3:
            ...
            return torch.einsum(
                'thpw,bwn->bthpn', attn_base, self.mapper)

    def __init__(
            self, prompts, num_steps: int, cross_replace_steps: float,
        ...
        super(
            ...
            latent_blend, tokenizer=tokenizer,
        ...
        self.mapper = seq_aligner.get_replacement_mapper(
            prompts, tokenizer, target_weight=target_weight).to(device)
    ...
    def replace_cross_attention(self, attn_base, att_replace):
        ...
        if attn_base.dim() == 3:
            attn_base_replace = attn_base[:, :,
                                          self.mapper].permute(2, 0, 1, 3)
        elif attn_base.dim() == 4:
            attn_base_replace = attn_base[:, :, :, self.mapper].permute(
                3, 0, 1, 2, 4)
...
class AttentionReweight(AttentionControlEdit):
    ...
    def __init__(
            ...
            tokenizer=None, additional_attention_store=None,
            use_inversion_attention=False,
            attention_blend: SpatialBlender = None,
            save_self_attention: bool = True, disk_store=False):
        super(
            AttentionReweight, self).__init__(
            prompts, num_steps, cross_replace_steps, self_replace_steps,
            latent_blend, tokenizer=tokenizer,
            additional_attention_store=additional_attention_store,
            use_inversion_attention=use_inversion_attention,
...
def get_equalizer(
        text: str, word_select: Union[int, Tuple[int, ...]],
...
def make_controller(tokenizer, prompts: List[str],
    ...
    if (blend_words is None) or (blend_words == 'None'):
        ...
        if blend_latents:
            latent_blend = SpatialBlender(
                prompts, blend_words, start_blend=0.2, end_blend=0.8,
                tokenizer=tokenizer, th=blend_th,
                NUM_DDIM_STEPS=NUM_DDIM_STEPS, save_path=save_path +
                f'/latent_blend_mask', prompt_choose='both')
            print(f'Blend latent mask with threshold {blend_th}')
        else:
            latent_blend = None
        if blend_self_attention:
            attention_blend = SpatialBlender(
                prompts, blend_words, start_blend=0.0, end_blend=2,
                tokenizer=tokenizer, th=blend_th,
                NUM_DDIM_STEPS=NUM_DDIM_STEPS, save_path=save_path +
                f'/attention_blend_mask', prompt_choose='source')
            print(
                f'Blend self attention mask with threshold {blend_th}')
        else:
            attention_blend = None
    ...
    if equilizer_params is not None:
        ...
        controller = AttentionReweight(
            ...
            latent_blend=latent_blend, controller=controller,
            tokenizer=tokenizer,
            additional_attention_store=additional_attention_store,
            use_inversion_attention=use_inversion_attention,
            attention_blend=attention_blend,
