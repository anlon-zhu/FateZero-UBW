...
class SpatialBlender:
    ...
    def get_mask(
            self, maps, alpha, use_pool, h=None, w=None, x_t=None,
        ...
        if h is None and w is None and x_t is not None:
            # record the shape if only latent is provided
            h, w = x_t.shape[-2:]
        k = 1
        ...
        steepness = 1.5  # TODO: pass configurable hyperparameter to yaml
    ...
    def __call__(
            self, attention_store, step_in_store: int = None,
            target_h=None, target_w=None, x_t=None):
        """
        ...
        if target_h is None and target_w is None and x_t is not None:
            # record the shape if only latent is provided
            target_h, target_w = x_t.shape[-2:]

        self.counter += 1
        # if (self.counter > self.start_blend) and (self.counter < self.end_blend):

        maps = attention_store["down_cross"][
            2: 4] + attention_store["up_cross"][
            : 3]
        # breakpoint()
        ...
        for item in maps:
            ...
            rearranged_item = rearrange(
                item, "p c h (res_h res_w) w -> p h c res_h res_w w ",
        ...
        mask = self.get_mask(
            maps, masked_alpah_layers, True, target_h, target_w,
            step_in_store=step_in_store)

        if self.substruct_layers is not None:
            maps_sub = ~self.get_mask(
                maps, self.substruct_layers, False)
            mask = mask * maps_sub
        mask = mask.float()
    ...
    def __init__(
            ...
            words: [List[List[str]]],
            substruct_words=None, start_blend=0.2, end_blend=0.8,
            th=(0.9, 0.9),
            tokenizer=None, NUM_DDIM_STEPS=None, save_path=None,
        ...
        if save_path is not None:
            ...
            self.save_path = None
        assert prompt_choose in [
            'source', 'both'], "choose to generate the mask by only source prompt or both the source and target"
        self.prompt_choose = prompt_choose
        alpha_layers = torch.zeros(
            len(prompts),
            1, 1, 1, 1, self.MAX_NUM_WORDS)
        ...
        if substruct_words is not None:
            substruct_layers = torch.zeros(
                len(prompts),
                1, 1, 1, 1, self.MAX_NUM_WORDS)
            for i, (prompt, words_) in enumerate(
                    zip(prompts, substruct_words)):
                if type(words_) is str:
                    words_ = [words_]
                for word in words_:
                    ind = ptp_utils.get_word_inds(
                        prompt, word, tokenizer)
        ...
        print(
            self.alpha_layers[0]
            [..., 0: (len(prompts[0].split(" ")) + 2)])
        print(
            self.alpha_layers[1]
            [..., 0: (len(prompts[1].split(" ")) + 2)])
